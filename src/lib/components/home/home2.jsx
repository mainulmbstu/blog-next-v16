...
import PostModal from "@/app/(frontend)/post/post-details/[pid]/PostModal";
import SubmitButton from "@/lib/components/SubmitButton";

const Home2 = async ({ searchParams }) => {
  ...
  return (
    <div className="p-2">
      <div>
        <PostModal title="Create post" design="btn-primary" />
      </div>
      <div className="my-3">
        <Form action={"/"}>
          <div className="flex ">
            <div className=" w-80">
              <input
                ...
                type="search"
                className="input"
                placeholder="Title or Author name or Post details "
              />
            </div>
            <div className="">
              <SubmitButton title={"Search"} design={" btn-search"} />
            </div>
          </div>
        </Form>
      </div>
